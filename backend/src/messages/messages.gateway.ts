...
export class MessagesGateway {
  ...
  async create(
    ...
    const { msg, chan } = await this.messageService.create(
      login,
      name,
      createMessageDto,
    );
  ...
  async createChan(
    ...
    @MessageBody('name') name: string,
    @MessageBody('type') type: ChanType,
    @MessageBody('password') password: string,
    @ConnectedSocket() client: Socket,
  ) {
    // need to tchek if name exist AND CRYPT PASSWORd
    const chan = await this.messageService.CreateChan(
      usr,
      name,
      type,
      password,
  ...
  async LeaveRoom(
    ...
    const chanPart = await myDataSource
    ...
    chanPart.forEach(async (element) => {
      if (element.participant && element.chan) {
        if (
          element.participant.login == user.login &&
          element.chan.name == name
        ) {
          const chanPartDelete = await myDataSource
            .getRepository(ChanParticipant)
            .findOneBy({ id: element.id });
  ...
  async userchanStatus(
    @MessageBody('name') name: string,
    @MessageBody('login') login: string,
  ) {
    const list = await myDataSource
      .getRepository(ChanParticipant)
      .find({ relations: ['participant', 'chan'] });
    list.forEach((element) => {
      if (element.chan && element.participant) {
        if (element.chan.name == name && element.participant.login == login)
          if (element.ban == true) return 'BAN';
        if (element.mute == true) return 'MUTE';
        if (element.privilege == ChanPartStatus.OWNER) return 'OWNER';
        if (element.privilege == ChanPartStatus.ADMIN) return 'ADMIN';
      }
    });
  }
  ...
  async MuteBanUser(
    ...
    @MessageBody() updateChanParticipantDto: UpdateChanParticipantDto,
    @ConnectedSocket() client: Socket,
  ) {
    const { arg, bool, chan } = await this.messageService.muteBanUser(
      name,
      updateChanParticipantDto,
      target,
    );
  ...
  async isPassword(
    ...
    const chan = await myDataSource
      .getRepository(Channel)
      .findOne({ where: { name: name } });
    if (bcrypt.compareSync(password, chan.password) == true) return true;
    return false;
  }
  ...
  async ListOfAdmin(@MessageBody('name') name: string) {
    ...
    list.forEach((element) => {
      if (element.chan && element.participant) {
        if (
          element.chan.name == name &&
          element.privilege == ChanPartStatus.ADMIN
        )
  ...
  async getUserChan(@MessageBody('name') name: string) {
    ...
    list.forEach((element) => {
      if (element.chan && element.participant) {
        if (
          element.chan.name == name &&
          element.privilege == ChanPartStatus.OWNER
        )
  ...
  async createDM(
    @MessageBody('user') user: User,
    @MessageBody('target') target: User,
    @ConnectedSocket() client: Socket,
  ) {
    // tcheker si il est blocker  dans les deux sens
    const { chan, targetsocket } = await this.messageService.createDM(
      user,
      target,
    );
      ...
      if (element.chan && element.participant) {
        if (
          ...
          list.forEach(async (element1) => {
            if (
              element1.chan.name == name &&
              element1.participant.login == login
            ) {
              chanPart = element1;
