...
export class GamesGateway {
  ...
  async handlePoint(
    ...
    const player = await myDataSource
  ...
  async handleFinishGame(
    ...
    const endPayload: EndGamePayload = {
      gameId: gameId,
  ...
  async handleLoopOutput(gameId: string, details: LoopDetails) {
    ...
    if (details.score) {
      let playerId: string;
      if (details.isP1Score) playerId = gameState.player1.id;
      else playerId = gameState.player2.id;
      this.handlePoint(
        gameId,
        playerId,
  ...
  async gameLoop(gameId: string) {
    const gameClock = setInterval(() => {
      ...
      if (!endGame) this.handleLoopOutput(gameId, details);
        ...
        const playerOneId: string =
          this.gamesService.getState(gameId).player1.id;
        const playerTwoId: string =
          this.gamesService.getState(gameId).player2.id;
